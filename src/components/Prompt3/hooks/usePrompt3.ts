...
interface FormData {
  ...
  const schema = z.object({
    resourcesAvailable: z.string().min(1, "Resources Available information is required"),
...
const usePrompt3 = (initialData?: FormData) => {
    ...
    const { mutate: postVisitorStatus } = usePostVisitorStatus(); 
    const {
      register,
      handleSubmit,
      formState: { errors },
